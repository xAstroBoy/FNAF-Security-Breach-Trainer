...
public:
	...
	TMap<struct FName, struct FFNAFMessageTableStruct> Read;                                                      // 0x0248(0x0050) (Edit, BlueprintVisible, DisableEditOnInstance)
	TMap<struct FName, struct FFNAFMessageTableStruct> New;                                                       // 0x0298(0x0050) (Edit, BlueprintVisible, DisableEditOnInstance)
	bool                                               HasUnread;                                                 // 0x02E8(0x0001) (Edit, BlueprintVisible, ZeroConstructor, DisableEditOnInstance, IsPlainOldData, NoDestructor)
	unsigned char                                      UnknownData_EA24[0x3];                                     // 0x02E9(0x0003) MISSED OFFSET (FIX SPACE BETWEEN PREVIOUS PROPERTY)
	...
	unsigned char                                      UnknownData_7BOH[0x4];                                     // 0x0304(0x0004) MISSED OFFSET (FIX SPACE BETWEEN PREVIOUS PROPERTY)
	...
	unsigned char                                      UnknownData_JZFX[0x4];                                     // 0x031C(0x0004) MISSED OFFSET (FIX SPACE BETWEEN PREVIOUS PROPERTY)
	...
	{
		static UClass* ptr = UObject::FindClass("WidgetBlueprintGeneratedClass WI_Messages.WI_Messages_C");
		return ptr;
	}
	...
	void ParseReadMessage(const struct FName& Name, const struct FFNAFMessageTableStruct& Data);
	void Check_Read();
	void PopulateList();
	void MarkAsRead(const struct FName& Name);
	void ParseNewMessage(const struct FName& Name, const struct FFNAFMessageTableStruct& Data);
	void InuputNext();
	void InputPrevious();
	void InputPrimary();
	void InputSecondary();
	...
	void BindAllEvents();
	void ObtainMessage(const struct FName& MessageName, const struct FFNAFMessageTableStruct& MessageInfo);
	void InputClose();
