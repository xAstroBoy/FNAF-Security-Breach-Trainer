...
// 0x0150 (FullSize[0x0380] - InheritedSize[0x0230])
...
public:
	...
	unsigned char                                      UnknownData_PIM9[0x7];                                     // 0x02E9(0x0007) MISSED OFFSET (FIX SPACE BETWEEN PREVIOUS PROPERTY)
	TArray<struct FName>                               TasksUpdated;                                              // 0x02F0(0x0010) (Edit, BlueprintVisible, DisableEditOnInstance, HasGetValueTypeHash)
	...
	{
		static UClass* ptr = UObject::FindClass("WidgetBlueprintGeneratedClass DebugMissionUI.DebugMissionUI_C");
		return ptr;
	}
	...
	void SetUpTaskActivateStates(const struct FName& TaskName);
	void AwardTaskItems(const struct FName& TaskName);
	void CompleteMissionTask(const struct FName& MissionName, int InfoState);
	void UpdateDependentMissions(const struct FName& MissionName, TArray<struct FName>* TasksUpdated);
	void AwardItems(TArray<struct FName>* Array);
